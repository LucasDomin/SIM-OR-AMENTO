      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <motion.header initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 border-b border-white/10 pb-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="font-display text-4xl text-white md:text-5xl">Price List</h1>
          ...
          <button onClick={saveSettings} className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-medium text-black transition hover:bg-white/90"><Save size={16} />Salvar settings</button>
        </motion.header>

        <section className="grid gap-4 md:grid-cols-3">
          <Setting label="Fee Percentage" value={settings.fee_percentage} onChange={(value) => setSettings({ ...settings, fee_percentage: value })} suffix="%" />
          <Setting label="Tax Percentage" value={settings.tax_percentage} onChange={(value) => setSettings({ ...settings, tax_percentage: value })} suffix="%" />
          <Setting label="Proposal Validity" value={settings.proposal_validity_days} onChange={(value) => setSettings({ ...settings, proposal_validity_days: value })} suffix="days" />
        </section>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-5">
          <div className="mb-5 flex flex-col gap-3 md:flex-row">
            ...
            <div className="flex gap-2 overflow-x-auto">{categories.map((cat) => <button key={cat} onClick={() => setCategory(cat)} className={`whitespace-nowrap rounded-xl px-3 py-2 text-xs transition ${category === cat ? 'bg-white text-black' : 'bg-white/5 text-white/45 hover:text-white'}`}>{cat === 'all' ? 'Todos' : cat}</button>)}</div>
          </div>

          <div className="space-y-2">
            {filtered.map((item) => <div key={item.id} className="grid gap-3 rounded-2xl border border-white/10 bg-black/25 p-4 md:grid-cols-[1fr_150px_150px_120px] md:items-center">
              <div><p className="text-sm font-medium text-white">{item.name}</p><p className="text-xs text-white/35">{item.category}</p></div>
              ...
              <div className="text-right text-xs text-white/35"><p>Spread</p><p className="text-sm text-white/70">{formatCurrency(item.sale_price - item.cost_price)}</p></div>
            </div>)}
          </div>
        </section>
      </div>
...
function Setting({ label, value, onChange, suffix }: { label: string; value: number; onChange: (value: number) => void; suffix: string }) {
  return <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><SlidersHorizontal size={16} className="mb-5 text-accent" /><p className="text-xs uppercase tracking-[0.24em] text-white/30">{label}</p><div className="mt-3 flex items-end gap-2"><input type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-24 bg-transparent font-display text-4xl text-white outline-none" /><span className="pb-2 text-sm text-white/35">{suffix}</span></div></div>;
}
